...
function setCookie(name: string, value: string, res: Response): void {
  res.headers.append(
    "Set-Cookie",
    `${name}=${value}; Path=/; SameSite=Lax; Max-Age=3600`
  );
}
...
async function handleEmbedCommentArea(
  areaKey: string,
  request: Request,
  env: Env,
  lang: string,
  ...
  if (!area) {
    console.log(
      `Comment area with key '${areaKey}' not found for embed. Attempting to create automatically.`
    );
    try {
      ...
      console.error(
        `Failed to auto-create comment area '${areaKey}' for embed:`,
        e
      );
      // 自動作成に失敗した場合はエラーを返す
      if (e instanceof Error) {
        return new Response(
          `Failed to create comment area automatically: ${e.message}`,
          { status: 500 }
        );
      } else {
        return new Response(
          `Failed to create comment area automatically: Unknown error`,
          { status: 500 }
        );
      }
    }
      ...
      <body data-theme="${theme}">
        ...
        <script>
          ...
          function checkFormValidity() {
            if (
              ...
              if (!newCommentInput.value.trim()) {
                ...
                submitTooltip.textContent =
                  "${t.turnstile_verification_required}"; // 日本語キーを使用
              }
            }
          }
          ...
          submitButton.addEventListener("click", async () => {
            ...
            const res = await fetch("/area/${areaKey}/comment", {
              method: "POST",
              body: formData,
            });
            if (res.ok) {
...
async function handleHomePage(
        ...
        <div id="adminSection" class="admin-section ${!authed ? "hidden" : ""}">
          ...
          <div class="form-group">
            ...
            <input
              type="text"
              id="areaKey"
              placeholder="${t.area_key_placeholder}"
            />
            <textarea
              id="areaIntro"
        ...
        <script>
          ...
          if (loginBtn) {
            loginBtn.addEventListener("click", async () => {
              ...
              if (!data.success) {
                showNotification(
                  data.message || "${t.notification_login_failed}"
                );
              } else {
                location.reload();
              }
          ...
          if (createAreaBtn) {
            createAreaBtn.addEventListener("click", async () => {
              ...
              if (res.ok) {
                ...
                showNotification(
                  "${t.notification_create_failed}：" + (await res.text())
                );
              }
            });
          }
 ...
 <tr>
     ...
     <td>
         <a href="/area/\${encodeURIComponent(
           a.area_key
 ...
 \`;
          ...
          async function deleteArea(areaKey) {
            ...
            if (res.ok) {
              ...
              showNotification(
                "${t.notification_delete_failed}：" + (await res.text())
              );
            }
          }
...
async function handleAdminExtendedInfo(
  request: Request,
  env: Env
): Promise<Response> {
  const cookie = parseCookie(request.headers.get("Cookie"));
...
/** 管理者が新しい議論エリアを作成する */
async function handleCreateCommentArea(
  request: Request,
  env: Env
): Promise<Response> {
  const cookie = parseCookie(request.headers.get("Cookie"));
...
async function handleCommentAreaPage(
  request: Request,
  env: Env,
  lang: string,
  theme: string
...
async function handleGetComments(
  ...
  const url = new URL(request.url);
  const areaKey = decodeURIComponent(
    url.pathname.replace(/^\/area\/|\/comments$/g, "")
  );
  // 議論エリアが非表示かどうかをチェック
  const area: { hidden: number } | null = await env.DB.prepare(
    "SELECT hidden FROM comment_areas WHERE area_key=?"
  )
    .bind(areaKey)
    .first<{ hidden: number }>();
...
async function handlePostComment(
  ...
  const url = new URL(request.url);
  const areaKey = decodeURIComponent(
    url.pathname.replace(/^\/area\/|\/comment$/g, "")
  );
  const formData = await request.formData();
  ...
  // コメントエリアの存在チェックと自動作成
  let area: { hidden: number } | null = await env.DB.prepare(
    "SELECT hidden FROM comment_areas WHERE area_key=?"
  )
    .bind(areaKey)
    .first<{ hidden: number }>();
...
async function handleDeleteArea(
  ...
  // まず存在するかチェック
  const area: { id: number } | null = await env.DB.prepare(
    "SELECT id FROM comment_areas WHERE area_key=?"
  )
    .bind(areaKey)
    .first<{ id: number }>();
  if (!area) {
  ...
  await env.DB.prepare("DELETE FROM comments WHERE area_key=?")
    .bind(areaKey)
    .run();
  await env.DB.prepare(
    "DELETE FROM reports WHERE comment_id IN (SELECT id FROM comments WHERE area_key=? )"
  )
    .bind(areaKey)
    .run(); // Add delete reports
  await env.DB.prepare("DELETE FROM comment_areas WHERE area_key=?")
    .bind(areaKey)
    .run();
  return new Response("OK", { status: 200 });
}
/** 議論エリアの非表示状態を切り替える (管理者) */
async function handleToggleHideArea(
  areaKeyEncoded: string,
  request: Request,
  env: Env
): Promise<Response> {
  ...
  await env.DB.prepare("UPDATE comment_areas SET hidden=? WHERE area_key=?")
    .bind(newHidden, areaKey)
    .run();
  return new Response("OK", { status: 200 });
}
...
async function handleTogglePinComment(
  ...
  // 存在するかどうかをクエリ
  const comment: { pinned: number } | null = await env.DB.prepare(
  ...
  await env.DB.prepare("UPDATE comments SET pinned=? WHERE id=?")
    .bind(newPinned, commentId)
    .run();
  return new Response("OK", { status: 200 });
}
/** レポートを処理する -> 処理済みとマーク */
async function handleResolveReport(
  ...
  // 存在するかどうかをクエリ
  const rep: { id: number } | null = await env.DB.prepare(
  ...
  await env.DB.prepare("UPDATE reports SET resolved=1 WHERE id=?")
    .bind(reportId)
    .run();
  return new Response("OK", { status: 200 });
}
